...
OUTPUT_DIR = "graficos_throughput"
CONFIDENCE = 0.95
...
def parse_algorithm(path: str):
    """
    Extrai apenas o algoritmo a partir do caminho do arquivo, ignorando
    a necessidade de procurar por rounds, já que agora é uma pasta única.
    """
...
def build_dataframe_from_caliper_reports(reports_dir: str) -> pd.DataFrame:
    ...
    Mesmo que seja uma única pasta chamada 'saturacao', o glob encontrará o HTML.
    """
...
def mean_and_ci(values: np.ndarray, confidence: float = CONFIDENCE):
    """
    Se houver apenas 1 execução na pasta de saturação, retorna margem de erro 0.0.
    Se houver múltiplas execuções, calcula o Intervalo de Confiança padrão.
    """
    n = len(values)
    mean = np.mean(values)
    if n < 2:
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    """
    Agrupa por (algorithm, workload, tps).
    """
    records = []
    grouped = df.groupby(["algorithm", "workload", "tps"])["throughput"]
...
def plot_workload(summary: pd.DataFrame, workload: str, output_dir: str) -> str:
    ...
    ax.set_title(f"Throughput vs TPS Alvo — {WORKLOAD_LABELS.get(workload, workload)}",
    ...
    out_path = os.path.join(output_dir, f"saturacao_throughput.png")
...
def main():
    ...
    required_cols = {"algorithm", "workload", "tps", "throughput"}
