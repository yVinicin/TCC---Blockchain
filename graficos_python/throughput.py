...
def parse_algorithm_and_round(path: str):
    ...
    Ajuste esta função conforme a organização real das suas pastas.

    Suporta, por padrão:
      .../Raft/round1/report.html
      .../raft/r3/report.html
      .../SmartBFT_round7/report.html
      .../smartbft-10/report.html
    """
...
def parse_report_html(html_path: str) -> list[dict]:
    ...
    for tr in rows[1:]:
        ...
        if not m:
            continue  # linha que não bate com o padrão esperado, ignora
...
def build_dataframe_from_caliper_reports(reports_dir: str) -> pd.DataFrame:
    ...
    if not html_paths:
        # também aceita um único arquivo direto na pasta, ou outros nomes *.html
...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    a partir dos 10 rounds de cada combinação.
    """
    records = []
...
def plot_workload(summary: pd.DataFrame, workload: str, output_dir: str) -> str:
    ...
    ax.set_ylabel("Throughput Médio (tx/s)", fontsize=11)
    ...
    out_path = os.path.join(output_dir, f"throughput_{workload}.png")
...
def main():
    ...
    required_cols = {"algorithm", "workload", "tps", "round", "throughput"}
